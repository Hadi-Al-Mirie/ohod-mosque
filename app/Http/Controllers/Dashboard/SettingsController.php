    ...
    public function update(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'result_settings.*.min_res' => [
                    'required',
                    'integer',
                    'between:0,100',
                ],
                'result_settings.*.max_res' => [
                    'required',
                    'integer',
                    'between:0,100',
                    'gte:result_settings.*.min_res',
                ],
                'result_settings.*.points' => ['required', 'numeric', 'min:0', 'max:100'],
                'level_mistakes' => ['required', 'array'],
                'level_mistakes.*' => ['required', 'array'],
                'level_mistakes.*.*' => ['required', 'integer', 'min:0', 'max:70'],
                'attendance_types' => ['required', 'array', 'size:4'],
                'attendance_types.*' => ['required', 'integer', 'min:-100', 'max:100'],
            ]);
            ...
            $validator->after(function ($v) use ($request) {
                ...
                foreach ([4, 8] as $autoId) {
                    if (!isset($submitted[$autoId]['min_res'])) {
                        $submitted[$autoId]['min_res'] = -10000;
                    }
                }
            ...
            $data['result_settings'][4]['min_res'] = -10000;
            $data['result_settings'][8]['min_res'] = -10000;
            // dd($data);
            ...
            Log::error('Error updating settings', ['error' => $ve->getMessage()]);
            return back()
                ->withInput()
                ->withErrors($ve->errors())
                ->with('danger', 'تأكد من صحة البيانات .');

        } catch (\Exception $e) {
            Log::error('Error updating settings', ['error' => $e->getMessage()]);
            return back()
                ->with('danger', 'حدث خطأ أثناء تحديث الاعدادات.');
        }
    }
}
