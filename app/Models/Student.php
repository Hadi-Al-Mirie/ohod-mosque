...
class Student extends Model
{
    ...
    public function calculatePoints(int $courseId = null): float
    {
        ...
        // Log::info("Student {$this->id} notesPoints: {$notesPoints}");
        ...
        $sabrBase = DB::table('sabrs')
            ...
            ->where('sabrs.course_id', $cid)
            ->where('sabrs.student_id', $this->id)
            ->select(DB::raw('SUM(rs.points) AS sum_points'))
            ->first();

        $sabrPoints = $sabrBase->sum_points ?? 0;
        // Log::info("Student {$this->id} sabrPoints: {$sabrPoints}");

        // 4) Recitation points via subquery + join to result_settings
        ...
        $recitationBase = DB::table('recitations')
            ...
            ->where('recitations.course_id', $cid)
            ->where('recitations.student_id', $this->id)
            ->select(DB::raw('SUM(rs.points) AS sum_points'))
            ->first();

        $recitationPoints = $recitationBase->sum_points ?? 0;
        // Log::info("Student {$this->id} recitationPoints: {$recitationPoints}");
        ...
        // Log::info("Student {$this->id} awqafPoints: {$awqafPoints}");
        // 5) Total & return
        $total = round($sabrPoints + $recitationPoints + $attendancePoints + $notesPoints + $awqafPoints, 2);
        // Log::info("Student {$this->id} totalPoints: {$total}");
    ...
    public function pointsUpTo(Carbon $cutoff): float
    {
        ...
        $sabrScoreSub = DB::table('sabrs')
            ...
            ->where('sabrs.student_id', $this->id)
            ->whereDate('sabrs.created_at', '<=', $cutoff)
            ->groupBy('sabrs.id');
        ...
        $sabrPoints = $sabrBase->sum_points;
        ...
        $recScoreSub = DB::table('recitations')
            ...
            ->where('recitations.student_id', $this->id)
            ->whereDate('recitations.created_at', '<=', $cutoff)
            ->groupBy('recitations.id');

        $recBase = DB::table('recitations')
            ->joinSub($recScoreSub, 'scores', 'scores.id', 'recitations.id')
            ->leftJoin('result_settings AS rs', function ($join) {
                $join->on('rs.type', DB::raw("'recitation'"))
                    ->on('rs.min_res', '<=', 'scores.raw_score')
        ...
        $recitationPoints = $recBase->sum_points;
        $awqafPoints = $this->awqafs()
            ->where('course_id', $courseId)
            ->where('type', 'success')
            ->whereDate('created_at', '<=', $cutoff)
    ...
    public function rankInCircle(Carbon $upto = null): int
    {
        ...
        return $sorted->search(fn($v, $i) => $students[$i]->id === $this->id) + 1;
    }
    ...
    public function rankInMosque(Carbon $upto = null): int
    {
        ...
        $sorted = $scores->sortDesc()->values();
        return $sorted->search(fn($v, $i) => $all[$i]->id === $this->id) + 1;
    }
